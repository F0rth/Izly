            ...
            if (parameterizedType2 != null) {
                ...
                for (int i = 0; i < actualTypeArguments.length; i++) {
                    ...
                    while (obj instanceof TypeVariable) {
                        type2 = (Type) r12.get(((TypeVariable) obj).getName());
                    }
                    r12.put(typeVariable.getName(), obj);
                }
                if (typeEquals(parameterizedType2, parameterizedType, r12)) {
                    return true;
                }
            }
            for (Type isAssignableFrom : rawType.getGenericInterfaces()) {
                if (isAssignableFrom(isAssignableFrom, parameterizedType, new HashMap(r12))) {
                    return true;
                }
            }
            type = rawType.getGenericSuperclass();
            Object hashMap = new HashMap(r12);
            type2 = type;
        }
        return false;
    }

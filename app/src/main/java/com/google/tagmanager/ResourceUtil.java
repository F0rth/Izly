    ...
    private static Value expandValue(int i, Resource resource, Value[] valueArr, Set<Integer> set) throws InvalidResourceException {
        ...
        switch (value.type) {
            ...
            case 2:
                ...
                newValueBasedOnValue = newValueBasedOnValue(value);
                newValueBasedOnValue.listItem = new Value[servingValue.listItem.length];
                iArr = servingValue.listItem;
                length = iArr.length;
                i3 = 0;
                while (i2 < length) {
                    newValueBasedOnValue.listItem[i3] = expandValue(iArr[i2], resource, valueArr, set);
                    i2++;
                    i3++;
                }
                break;
            case 3:
                Value newValueBasedOnValue2 = newValueBasedOnValue(value);
                ...
                newValueBasedOnValue2.mapKey = new Value[servingValue2.mapKey.length];
                newValueBasedOnValue2.mapValue = new Value[servingValue2.mapKey.length];
                int[] iArr2 = servingValue2.mapKey;
                int length2 = iArr2.length;
                i3 = 0;
                int i4 = 0;
                while (i3 < length2) {
                    newValueBasedOnValue2.mapKey[i4] = expandValue(iArr2[i3], resource, valueArr, set);
                    i3++;
                    i4++;
                }
                ...
                i3 = 0;
                while (i2 < length) {
                    newValueBasedOnValue2.mapValue[i3] = expandValue(iArr3[i2], resource, valueArr, set);
                    i2++;
                    i3++;
                }
                value2 = newValueBasedOnValue2;
                break;
            case 4:
            ...
            case 7:
                newValueBasedOnValue = newValueBasedOnValue(value);
                servingValue = getServingValue(value);
                newValueBasedOnValue.templateToken = new Value[servingValue.templateToken.length];
                iArr = servingValue.templateToken;
                length = iArr.length;
                i3 = 0;
                while (i2 < length) {
                    newValueBasedOnValue.templateToken[i3] = expandValue(iArr[i2], resource, valueArr, set);
                    i2++;
                    i3++;
                }
                break;

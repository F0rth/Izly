...
class UniversalAnalyticsTag extends TrackingTag {
    ...
    private Map<String, String> getTransactionItemFields(Map<String, Value> map) {
        ...
        if (defaultItemMap == null) {
            ...
            hashMap.put(Item.KEY_CATEGORY, Fields.ITEM_CATEGORY);
            hashMap.put(Item.KEY_PRICE, Fields.ITEM_PRICE);
            hashMap.put(Item.KEY_QUANTITY, Fields.ITEM_QUANTITY);
    ...
    private void sendTransaction(Tracker tracker, Map<String, Value> map) {
        ...
        try {
            ...
            if (transactionItems != null) {
                for (Map convertToGaFields2 : transactionItems) {
                    if (convertToGaFields2.get("name") == null) {
                        Log.e("Unable to send transaction item hit due to missing 'name' field.");
                        return;
                    }
                    Map convertToGaFields3 = convertToGaFields((Value) map.get(ANALYTICS_FIELDS));
                    convertToGaFields3.put(Fields.HIT_TYPE, HitTypes.ITEM);
                    convertToGaFields3.put(Fields.TRANSACTION_ID, dataLayerString);
                    for (Entry entry2 : getTransactionItemFields(map).entrySet()) {
                        addParam(convertToGaFields3, (String) entry2.getValue(), (String) convertToGaFields2.get(entry2.getKey()));
                    }
                    linkedList.add(convertToGaFields3);
                }
            }

...
class ResourceLoader implements Runnable {
    ...
    ResourceLoader(Context context, String str, NetworkClientFactory networkClientFactory, CtfeHost ctfeHost) {
        ...
        this.mDefaultCtfeUrlPathAndQuery = new StringBuilder(CTFE_URL_PREFIX).append(str).toString();
        ...
        if (okToLoad()) {
            ...
            try {
                ...
                try {
                    ...
                    Log.w("Error when parsing downloaded resources from url: " + ctfeUrl + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + e.getMessage(), e);
                ...
                Log.w("Error when loading resources from url: " + ctfeUrl + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + e3.getMessage(), e3);
    ...
    String getCtfeUrl() {
        ...
        return PreviewManager.getInstance().getPreviewMode().equals(PreviewMode.CONTAINER_DEBUG) ? str + "&gtm_debug=x" : str;
    }

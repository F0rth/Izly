...
final class MultiDexExtractor {
    ...
    private static List<File> performExtractions(File file, File file2) throws IOException {
        ...
        int i = 2;
        ZipEntry entry = zipFile.getEntry(new StringBuilder(DEX_PREFIX).append(2).append(DEX_SUFFIX).toString());
        while (entry != null) {
            ...
            if (z) {
                int i4 = i + 1;
                i = i4;
                entry = zipFile.getEntry(new StringBuilder(DEX_PREFIX).append(i4).append(DEX_SUFFIX).toString());
            } else {

...
public class JDKKeyStore extends KeyStoreSpi implements BCKeyStore {
    ...
    public static class BouncyCastleStore extends JDKKeyStore {
        public void engineLoad(InputStream inputStream, char[] cArr) throws IOException {
            ...
            if (inputStream != null) {
                ...
                if (readInt == 2 || readInt == 0 || readInt == 1) {
                    ...
                    InputStream cipherInputStream = new CipherInputStream(dataInputStream, makePBECipher(readInt == 0 ? "OldPBEWithSHAAndTwofish-CBC" : JDKKeyStore.STORE_CIPHER, 2, cArr, bArr, readInt2));
        ...
        public void engineStore(OutputStream outputStream, char[] cArr) throws IOException {
            ...
            OutputStream cipherOutputStream = new CipherOutputStream(dataOutputStream, makePBECipher(JDKKeyStore.STORE_CIPHER, 1, cArr, bArr, nextInt));
        ...
        Object getObject(char[] cArr) throws NoSuchAlgorithmException, UnrecoverableKeyException {
            ...
            if (this.type == 4) {
                ...
                try {
                    ...
                    int readInt = dataInputStream.readInt();
                    try {
                        access$100 = JDKKeyStore.this.decodeKey(new DataInputStream(new CipherInputStream(dataInputStream, JDKKeyStore.this.makePBECipher("BrokenPBEWithSHAAnd3-KeyTripleDES-CBC", 2, cArr, bArr, readInt))));
                    } catch (Exception e2) {
                        dataInputStream = new DataInputStream(new ByteArrayInputStream((byte[]) this.obj));
                        bArr = new byte[dataInputStream.readInt()];
                        dataInputStream.readFully(bArr);
                        readInt = dataInputStream.readInt();
                        access$100 = JDKKeyStore.this.decodeKey(new DataInputStream(new CipherInputStream(dataInputStream, JDKKeyStore.this.makePBECipher("OldPBEWithSHAAnd3-KeyTripleDES-CBC", 2, cArr, bArr, readInt))));
                    }
                    if (access$100 != null) {
                        OutputStream byteArrayOutputStream = new ByteArrayOutputStream();
                        OutputStream dataOutputStream = new DataOutputStream(byteArrayOutputStream);
                        ...
                        DataOutputStream dataOutputStream2 = new DataOutputStream(new CipherOutputStream(dataOutputStream, JDKKeyStore.this.makePBECipher(JDKKeyStore.KEY_CIPHER, 1, cArr, bArr, readInt)));
                        JDKKeyStore.this.encodeKey(access$100, dataOutputStream2);
                        dataOutputStream2.close();
                        this.obj = byteArrayOutputStream.toByteArray();
                        return access$100;
                    }

...
public class MD2Digest implements ExtendedDigest {
    ...
    protected void processBlock(byte[] bArr) {
        ...
        while (i2 < 18) {
            ...
            for (i3 = 0; i3 < 48; i3++) {
                byte[] bArr2 = this.X;
                byte b = (byte) (S[i] ^ bArr2[i3]);
                bArr2[i3] = b;
                i = b & CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV;
            }
            i3 = (i + i2) % 256;
            i2++;
            i = i3;
    ...
    protected void processCheckSum(byte[] bArr) {
        ...
        for (int i2 = 0; i2 < 16; i2++) {
            byte[] bArr2 = this.C;
            bArr2[i2] = (byte) (S[(i ^ bArr[i2]) & CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV] ^ bArr2[i2]);
            i = this.C[i2];
        }
    }
    ...
    public void update(byte[] bArr, int i, int i2) {
        ...
        int i4 = i2;
        while (i4 > 16) {
            System.arraycopy(bArr, i3, this.M, 0, 16);
            processCheckSum(this.M);
            processBlock(this.M);
            i4 -= 16;
            i3 += 16;
